...
pub async fn get_ptn_string(details: &str) -> Result<String> {
...
pub async fn find_one_tinue<T: TakBoard + std::fmt::Debug + Send + 'static>(
    board: T,
    context: &Context,
    message: &Message,
) -> Result<()> {
...
pub fn find_all_tinue<T: TakBoard + std::fmt::Debug>(
    mut board: T,
    moves: &[GameMove],
) -> Vec<TinueStatus> {
    let mut vec = Vec::new();
...
pub fn parse_game(full_ptn: &str) -> Option<(TakGame, Vec<GameMove>)> {
